...
public sealed class UserTipsService
{
    ...
    private const string PathIconTipKey = "TipShown_PathIconSizing";
    ...
    /// <summary>
    /// True the first time the user copies a XAML PathIcon snippet; false
    /// afterwards. Marks the tip as seen as a side effect.
    /// </summary>
    public bool ShouldShowPathIconTip()
    {
        if (Settings.Values.ContainsKey(PathIconTipKey))
        {
            return false;
        }

        Settings.Values[PathIconTipKey] = true;
        return true;
    }
}
